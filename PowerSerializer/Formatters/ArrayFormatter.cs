...
public sealed class ArrayFormatter<A> : IFormatter<A> where A : notnull
{
    ...
    public ArrayFormatter(PowerSerializer serializer)
    {
        ...
        var concreteFormatterType = typeof(ConcreteFormatter<>).MakeGenericType([typeof(A), typeof(A).GetElementType()!]);
        _concreteFormatter = (IFormatter<A>)Activator.CreateInstance(concreteFormatterType, serializer)!;
    }
    ...
    private sealed class ConcreteFormatter<T> : IFormatter<A>
    {
        /// <summary>
